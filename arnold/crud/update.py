...
def update_sample(adapter: ArnoldAdapter, sample: LimsSample) -> str:
    ...
    result: UpdateResult = adapter.sample_collection.update_one(
        {"_id": sample_id},
        {"$set": sample.model_dump(by_alias=True, exclude_none=True)},
        upsert=True,
    )
    if result.raw_result.get("updatedExisting"):
        LOG.info(f"Updated step {sample_id}")
    else:
        LOG.info(f"Added step {sample_id}")
...
def update_step(adapter: ArnoldAdapter, step: Step) -> str:
    ...
    result: UpdateResult = adapter.step_collection.update_one(
        {"_id": step_id},
        {"$set": step.model_dump(by_alias=True, exclude_none=True)},
        upsert=True,
    )
    if result.raw_result.get("updatedExisting"):
        LOG.info(f"Updated step {step_id}")
    else:
        LOG.info(f"Added step {step_id}")
    return result.upserted_id
