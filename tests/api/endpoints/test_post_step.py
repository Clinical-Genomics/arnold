...
def test_post_valid_step(fast_app_client, valid_step):
    ...
    response = fast_app_client.post(
        "/api/v1/step/", json=valid_step.model_dump(by_alias=True)
    )
...
def test_post_step_already_in_db(mocker, fast_app_client, valid_step):
    ...
    mocker.patch("arnold.crud.read.step.get_step")
...
def test_post_step_create_step_failing(mocker, fast_app_client, valid_step):
    ...
    mocker.patch("arnold.crud.read.step.get_step", return_value=False)
    mocker.patch(
