...
def test_post_valid_step(fast_app_client, valid_load_step):
    # GIVEN valid step request data
    # WHEN running the user request with the data
    response = fast_app_client.post("/api/v1/step/", json=valid_load_step.dict())

    # THEN assert status is ok
    assert response.status_code == status.HTTP_200_OK


def test_post_invalid_step(fast_app_client, invalid_load_step):
    # GIVEN invalid step request data

    # WHEN running the user request with the data
    response = fast_app_client.post("/api/v1/step/", json=invalid_load_step)

    # THEN assert status 422
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_post_step_already_in_db(mocker, fast_app_client, valid_load_step):
...
def test_post_step_create_step_failing(mocker, fast_app_client, valid_load_step):
